        ...
        <div className="animate-in">
            ...
            {!projects?.length ? (
                <div className="glass-card rounded-3xl py-24 text-center">
                    <div
                        className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-white/5 text-slate-500 mb-4">
                    ...
                    {projects.map((project) => {
                        ...
                        return (
                            <Link
                                ...
                                className="group glass-card relative block overflow-hidden rounded-3xl p-6 transition-all hover:-translate-y-1 hover:border-cyan-500/30 hover:shadow-2xl hover:shadow-cyan-500/10 active:scale-[0.99]"
                            >
                                <div
                                    className="absolute -right-4 -top-4 h-24 w-24 rounded-full bg-cyan-500/5 blur-3xl transition-colors group-hover:bg-cyan-500/10"/>

                                <div className="mb-4 flex items-start justify-between gap-3">
        <span
            className={`shrink-0 rounded-full border px-3 py-1 text-[10px] font-bold uppercase tracking-wider ${status.color}`}
        >
          {status.label}
        </span>

                                    <span
                                        className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-slate-400 transition group-hover:border-cyan-400/30 group-hover:text-cyan-300">
          Открыть
        </span>
                                </div>

                                <div className="mb-6">
                                    <h3 className="line-clamp-2 text-xl font-bold leading-tight text-white transition-colors group-hover:text-cyan-400">
                                        {project.name}
                                    </h3>
                                </div>
          ...
          <span className="flex items-center gap-2">
            ...
            Задачи
                                        ...
                                        <span className="font-bold text-cyan-400">{taskCount}</span>
                                    </div>
                                </div>
                            </Link>
                        )
